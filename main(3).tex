...
予以接受，其中 \(\tau\) 为温度参数。退火结束后，再针对最长路线执行确定性平衡修复，并保留全部随机起点中 \(T_{\max}\) 最小的方案。若当前 \(N\) 找到满足 \(T_{\max}\leq9\) 的路线，则记录结果并继续优化；否则令 \(N\leftarrow N+1\)。最后独立复算各路线，检查巡检次数、同点连续访问、基地闭合和9小时限制。
各架无人机的正式巡检点序列按赛题模板写入
\texttt{result1.xlsx}；基地闭合路线、飞行距离、巡检次数、
工作时间和校验结果保存在
\nolinkurl{question1/results/solution.json} 中，
扩展工作簿仅作为复核材料保存。
...
\begin{center}
  \begin{minipage}[t]{0.49\textwidth}
    \centering
    \captionof{table}{问题一与问题二工作时间极差对比}
    ...
    \par\smallskip
    \parbox{\linewidth}{\footnotesize 注：极差及下降比例均由程序输出的未舍入数据计算，表中分钟数仅作显示舍入。}
  \end{minipage}
  \hfill
  \begin{minipage}[t]{0.47\textwidth}
    \centering
    \vspace{0pt}
    \includegraphics[width=\linewidth]{generated_figures/q2_balance_improvement_dumbbell.pdf}
    \captionof{figure}{问题一与问题二工作时间极差对比}
...
正文不逐行展开较长的巡检点序列。四个算例中每架无人机的正式巡检点序列
按赛题模板保存于 \texttt{result2.xlsx}；基地闭合路线、飞行距离、
巡检次数、工作时间和目标函数值保存在
\nolinkurl{question2/results/q2_solution.json} 及相应CSV文件中。
扩展工作簿仅作为复核材料保存于
\nolinkurl{audit_materials/result2_detailed.xlsx}。
问题二程序入口为
\nolinkurl{question2/program/q2_solver.py}。
...
算法B在固定机队规模 $N_0$ 下的正式巡检点序列按赛题模板保存于
\texttt{result3.xlsx}。算法B的逐事件时间表、固定机队正式结果及
12组机队规模敏感性结果保存在
\nolinkurl{question3/results/q3_solution.json}；
算法A对照结果保存在
\nolinkurl{question3/results/q3_solution_algorithmA.json}。
两种算法的扩展工作簿分别保存于
\nolinkurl{audit_materials/result3_detailed_algorithmA.xlsx}
和
\nolinkurl{audit_materials/result3_detailed_algorithmB.xlsx}，
仅用于结果复核，不作为赛题正式结果文件，避免两套程序和结果混淆。
...
\begin{table}[H]
  ...
  \begin{tabularx}{0.99\textwidth}{c c *{5}{>{\centering\arraybackslash}X}}
    ...
    Case 2 & 3 & 11.9325 & 11.9436 & 0.0081 & 448.214 & 233.445 \\
    Case 2 & 4 & 11.0787 & 11.0969 & 0.0108 & 952.904 & 269.180 \\
...
总体上，当前搜索得到的 $T_{\max}$ 随机队规模增加而下降，其中 Case 2 对增加无人机最敏感：由2架增加至3架和4架后，完成时间分别再缩短约71.8分钟和50.8分钟。表中的总等待是所有无人机等待时间之和，而 $T_{\max}$ 只由最晚返回的一架无人机决定；增机后更多并行路线进入管制时间窗，等待分散在多架无人机上，因而总等待增加并不必然导致系统完成时间增加。Case 2 的单机最大等待虽由34.612分钟增至233.445分钟和269.180分钟，但并行作业仍使最后返回时刻显著提前。$\delta$ 并不随 $N$ 单调变化，这是两阶段效率阈值、任务离散性和启发式搜索共同作用的结果。由于题目没有给出新增无人机成本或数量上限，不能仅凭该表定义无条件的“最优机队规模”，故仍以固定 $N_0$ 的方案作为正式答案。
...
\begin{longtable}{
  ...
  2 & \nolinkurl{requirements.txt} & Python依赖、Conda环境及问题一搜索参数。\\
  3 & \nolinkurl{reproduce_all.py}、\nolinkurl{formal_excel.py}、\nolinkurl{result1.xlsx}、\nolinkurl{result2.xlsx}、\nolinkurl{result3.xlsx}& 三问正式结果统一校验、赛题模板生成程序及三个唯一正式结果文件。\\
  ...
  6 & \nolinkurl{question1/results/} & 问题一正式结构化路线、搜索状态和校验记录。\\

  7 & \nolinkurl{question2/program/q2_solver.py} & 问题二负载均衡求解与独立校验程序；完整代码见附录C。\\
  8 & \nolinkurl{question2/results/} & 问题二结构化方案、汇总指标和逐架路线。\\
  ...
  11 & \nolinkurl{question3/program/q3_fast_validator.py}、\nolinkurl{question3/program/q3_enhanced_validator.py} & 算法A快速校验与算法B逐事件独立复核；完整代码见附录D。\\
