...
予以接受，其中 \(\tau\) 为温度参数。退火结束后，再针对最长路线执行确定性平衡修复，并保留全部随机起点中 \(T_{\max}\) 最小的方案。若当前 \(N\) 找到满足 \(T_{\max}\leq9\) 的路线，则记录结果并继续优化；否则令 \(N\leftarrow N+1\)。最后独立复算各路线，检查巡检次数、同点连续访问、基地闭合和9小时限制，并将详细调度方案写入 \texttt{result1.xlsx}。固定 \(N\) 时的完整优化流程移至附录，见图~\ref{fig:q1-route-flow}。
...
问题三另由独立程序重新读取原始附件，并逐事件复核唯一任务副本、基地闭合、时间连续性、每条折线航段的穿圆时段、管制区内服务和非基地安全等待，同时核对 $T_{\max}$、$T_{\min}$、$\delta$ 及两阶段效率阈值。算法B在4个算例、每个算例3种机队规模下形成的12组结果全部通过验证。该结论证明输出方案满足程序所实现的全部约束，但不等同于对全局最优性的证明；独立验证程序见附录~\ref{lst:q3-enhanced-validator}，算法A的快速解析校验器见附录~\ref{lst:q3-fast-validator}。
...
\section{模型评价与推广}
\subsection{模型优点}
本文将三级巡检次数要求转化为可核验的任务副本，在统一框架中处理机队规模、完成时间和负载均衡三个层次；问题三进一步以绝对时刻连接航段穿圆、圆内服务和安全等待约束。两阶段目标明确限制均衡优化造成的效率损失，独立校验器又从原始数据重新逐事件复算，使论文模型、程序输出和结果文件之间能够相互核对。算法A与算法B并列保留，也便于区分“直飞等待即可处理”的简单情形和“空间绕飞具有明显收益”的管制密集情形。

\subsection{模型不足与改进}
本文所得结果属于有限计算预算内的最好可行方案，尚未给出全局最优性证明。算法B取 $n_b=16$，对应22.5度的圆周角分辨率，在可见图规模与绕行精度之间作折中；虽然外偏半径保证采样弦不进入原禁飞圆，连续障碍最短路仍被离散近似。增加 $n_b$ 可提高空间分辨率，但会显著扩大时变图和候选边数量，后续可比较 $n_b=8,16,32$ 的完成时间与运行时间，或改用解析切线、自适应加密节点。其次，$\varepsilon_T=0.005$ 是效率与均衡之间的工程取舍，可进一步对 $\varepsilon_T=0,0.001,0.0025,0.005$ 做参数试验，绘制 $T_{\max}$--$\delta$ 权衡曲线。若实际系统还包含电量、速度波动或管制信息临时更新，则需扩展为带续航和不确定信息的滚动时域模型。

% =============================================================
% 参考文献：正式写作时取消下面三行注释，并在正文使用 \cite{键值}
% =============================================================
\section*{参考文献}
...
% =============================================================
% AI 使用披露：必须位于参考文献之后，并按专项规定如实填写
% =============================================================
\section*{人工智能工具使用披露}
\addcontentsline{toc}{section}{人工智能工具使用披露}
本队在论文撰写和程序整理过程中使用了 OpenAI Codex（GPT-5系列模型，2026年8月访问）作为辅助工具，主要用于检查 LaTeX 结构与排版、梳理模型表述、辅助编写和调试 Python 程序、整理结果工作簿以及检查正文与程序输出的一致性。人工智能工具未直接替代建模决策和结果判定：题目数据、模型约束、算法参数及数值结论均由队员结合赛题逐项确认；问题一、问题二结果由独立校验模块复算，问题三全部12组“算例--机队规模”结果由独立逐事件验证器重新读取原始附件，检查任务次数、基地闭合、时间连续性、动态管制冲突和效率阈值。论文最终内容、引用、程序及提交文件均由队员人工审阅并承担责任。

% =============================================================
% 附录：正文之后；页数不限；继续显示控制号和连续页码
% =============================================================
\clearpage
\appendix
...
\begin{longtable}{
  p{0.05\textwidth}
  >{\raggedright\arraybackslash\ttfamily\footnotesize}p{0.39\textwidth}
  >{\raggedright\arraybackslash\footnotesize}p{0.46\textwidth}
}
  \toprule
  序号 & 文件名 & 文件用途与对应论文位置 \\
  \midrule
  ...
  \midrule
  \endhead
  1 & \path{question1/program/} & 问题一完整程序、配置、输入读取、精确模型、启发式求解和独立校验模块，对应正文5.2节 \\
  2 & \path{question1/results/result1.xlsx} & 问题一四个算例的详细任务分配与巡检路线，对应表~\ref{tab:q1-results} \\
  3 & \path{question1/results/solution.json} & 问题一完整结构化路线和求解状态，也是问题二唯一使用的问题一初始调度输入 \\
  4 & \path{question2/program/q2_solver.py} & 固定问题一无人机数量和完成时间上界的负载均衡优化及独立校验程序，对应正文6.2节 \\
  5 & \path{question2/results/result2.xlsx} & 问题二各无人机任务序列、飞行距离、巡检次数和工作时间，对应表~\ref{tab:q2-results} \\
  6 & \path{question2/results/q2_solution.json} & 问题二可复算的完整结构化调度结果及随机搜索记录 \\
  7 & \path{question3/program/q3_solver.py} & 问题三基准算法A：直飞、必要安全等待和任务序列优化，对应正文7.2节 \\
  8 & \path{question3/program/q3_solver_enhanced.py} & 问题三改进算法B：时变可见图、空间绕飞和 $\varepsilon$-约束两阶段优化，对应正文7.2节 \\
  9 & \path{question3/results/result3_algorithmA.xlsx} & 算法A固定 $N_0$ 的基准路线与逐事件结果，对应表~\ref{tab:q3-ab-comparison} \\
  10 & \path{question3/results/q3_solution_algorithmA.json} & 算法A完整结构化基准结果及搜索记录 \\
  11 & \path{question3/results/result3.xlsx} & 算法B正式方案和12组机队规模敏感性结果，对应表~\ref{tab:q3-results}、\ref{tab:q3-fleet-sensitivity} \\
  12 & \path{question3/results/q3_solution.json} & 算法B完整结构化结果源，包含路线、事件时刻、两阶段搜索记录和独立验证状态 \\
  13 & \path{README.md}\newline\path{requirements.txt} & 三问目录说明、运行命令和 Python 依赖环境 \\
  \bottomrule
\end{longtable}

\noindent\textbf{提交说明：}支撑材料压缩包应包含上表列出的全部文件，并保证文件名、程序输出和论文中的表图一致；赛题提供的原始数据不必重复提交。

\section{问题一完整算法流程}
正文图~\ref{fig:q1-fleet-flow} 给出了无人机数量搜索的总体过程。固定无人机数量后，程序依次完成多起点初始路线构造、模拟退火搜索和确定性平衡修复，完整流程见图~\ref{fig:q1-route-flow}。
...
\begin{figure}[H]
  ...
  \includegraphics[
    ...
    keepaspectratio
  ]{figures/q1_route_optimization_flowchart_revised.pdf}
  \caption{固定无人机数量下的多起点路线优化流程}
  \label{fig:q1-route-flow}
\end{figure}

\section{完整源程序}
为保证计算结果可复现，本节应收入建模所使用的全部完整、可运行源程序。附录图~\ref{fig:q1-route-flow} 仅用于说明算法结构，不能替代源程序代码；相同程序文件还应放入单独提交的支撑材料压缩包。

\lstinputlisting[
  language=Python,
  caption={问题一多无人机协同巡检路径优化主程序},
  label={lst:q1-solver}
]{submission_package/question1/program/run.py}

\lstinputlisting[
  language=Python,
  caption={问题一多起点启发式与路线优化模块}
]{submission_package/question1/program/src/uav_q1/solver_engine.py}

\lstinputlisting[
  language=Python,
  caption={问题一固定无人机数量的精确模型模块}
]{submission_package/question1/program/src/uav_q1/exact_milp.py}

\lstinputlisting[
  language=Python,
  caption={问题一独立结果校验模块}
]{submission_package/question1/program/src/uav_q1/validation.py}

\lstinputlisting[
  language=Python,
  caption={问题一结果工作簿输出模块}
]{submission_package/question1/program/src/uav_q1/excel_output.py}

\lstinputlisting[
  language=Python,
  caption={问题二固定完成时间上界的工作负载均衡完整源程序},
  label={lst:q2-solver}
]{submission_package/question2/program/q2_solver.py}

\lstinputlisting[
  language=Python,
  caption={问题三基准算法A完整源程序},
  label={lst:q3-solver-a}
]{submission_package/question3/program/q3_solver.py}

\lstinputlisting[
  language=Python,
  caption={问题三改进算法B完整源程序},
  label={lst:q3-solver-b}
]{submission_package/question3/program/q3_solver_enhanced.py}

\lstinputlisting[
  language=Python,
  caption={问题三基于包围盒预筛和解析相交的加速独立验证程序},
  label={lst:q3-fast-validator}
]{submission_package/question3/program/q3_fast_validator.py}

\lstinputlisting[
  language=Python,
  caption={问题三算法B的逐事件独立验证程序},
  label={lst:q3-enhanced-validator}
]{submission_package/question3/program/q3_enhanced_validator.py}

\lstinputlisting[
  language=Python,
  caption={问题三Case 2算法A/B实际航迹对比图生成程序}
]{submission_package/question3/program/make_q3_case2_comparison.py}
